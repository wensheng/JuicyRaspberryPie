...
# Modified for JuicyRaspberryPie
# remove all dependencies on mcpi (minecraft, block, util, Vec3)
...
class MinecraftDrawing:
    ...
    def drawFace(self, vertices, filled, blockType):
        """
        ...
        if (filled):
            ...
            #draw wireframe
            self.drawVertices(edgesVertices, blockType)
    ...
    def drawCircle(self, x0, y0, z, radius, blockType):
        """
    ...
    def drawHorizontalCircle(self, x0, y, z0, radius, blockType):
        """
        ...
        x = 0
        z = radius
        self.drawPoint3d(x0, y, z0 + radius, blockType)
        self.drawPoint3d(x0, y, z0 - radius, blockType)
        self.drawPoint3d(x0 + radius, y, z0, blockType)
        self.drawPoint3d(x0 - radius, y, z0, blockType)
        ...
        while x < z:
            ...
            ddf_x += 2
            f += ddf_x   
            self.drawPoint3d(x0 + x, y, z0 + z, blockType)
            self.drawPoint3d(x0 - x, y, z0 + z, blockType)
            self.drawPoint3d(x0 + x, y, z0 - z, blockType)
            self.drawPoint3d(x0 - x, y, z0 - z, blockType)
            self.drawPoint3d(x0 + z, y, z0 + x, blockType)
            self.drawPoint3d(x0 - z, y, z0 + x, blockType)
            self.drawPoint3d(x0 + z, y, z0 - x, blockType)
            self.drawPoint3d(x0 - z, y, z0 - x, blockType)
...
class MinecraftShape:
    ...
    def __init__(self, mc, position, shapeBlocks = None, visible = True):
        ...
        self.originalPos = Vec3(position.x, position.y, position.z)
    ...
    def _copyBlocks(self, shapeBlocks):
        ...
        for shapeBlock in shapeBlocks:
            ...
            newShapeBlock.originalPos = Vec3(shapeBlock.originalPos.x, shapeBlock.originalPos.y, shapeBlock.originalPos.z)
            newShapeBlock.relativePos = Vec3(shapeBlock.relativePos.x, shapeBlock.relativePos.y, shapeBlock.relativePos.z)
...
class ShapeBlock():
    ...
    def __init__(self, x, y, z, blockType, blockData = 0, tag = ""):
        ...
        # the mc block object
        self.mcBlock = blockType

    def resetRelativePos(self):
        """
        resets the relative position of the block back to its original position
        """
        self.relativePos = Vec3(self.originalPos.x, self.originalPos.y, self.originalPos.z)
...
class MinecraftTurtle:
    ...
    def __init__(self, mc, position=Vec3(0, 0, 0), showTurtle=True):
        # set defaults
        self.mc = mc
        # start position
        ...
        if self.showturtle:
            self.turtle = self.mc.spawnEntity(self.position.x, self.position.y, self.position.z, "turtle")
        else:
            self.turtle = None
    ...
    def _moveTurtle(self, x, y, z):
        ...
        if self.turtlespeed == 0 and self.flying:
            ...
            for blockBetween in blocksBetween:
                ...
                if self.showturtle:
                    ...
                    #self.turtle.setPos(blockBetween.x, blockBetween.y, blockBetween.z)
                    self.mc.entity.setTilePos(self.turtle, blockBetween.x, blockBetween.y, blockBetween.z)
                # draw the pen
                if self._pendown:
        ...
        if self.showturtle:
            ...
            self.mc.entity.setTilePos(self.turtle, targetX, targetY, targetZ)
    ...
    def setposition(self, x, y, z):
        ...
        if self.showturtle:
            ...
            self.mc.entity.setTilePos(self.turtle, self.position.x, self.position.y, self.position.z)
    ...
    def _drawTurtle(self, x, y, z):
        ...
        lastDrawnTurtle = Vec3(x, y, z)

    def _clearTurtle(self, x, y, z):
        # clear turtle
        self.mcDrawing.drawPoint3d(x, y, z, "air")
