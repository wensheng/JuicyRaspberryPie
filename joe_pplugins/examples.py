...
def cube(size=5, typeId=1, data=0):
    "_mcp: create a cube"
    mc = Minecraft.create()
    ...
    mc.setBlocks(pos.x + 1, pos.y, pos.z,
                 pos.x + s, pos.y + s - 1, pos.z + s - 1, t, data)
    

def flatten(size=50,blktype=24):
    ...
    mc.setBlocks(pos.x - s, pos.y - 1, pos.z - s,
                 pos.x + s, pos.y - 1, pos.z + s, blktype)
...
def banner(txt, size=24, type1=41, type2=0):
    ...
    type1 = int(type1)
    type2 = int(type2)
    w2b = pplugins.word2banner.word2banner(txt, 4, size)
    y = pos.y + size
    for r in w2b:
        z = pos.z + 1
        for c in r:
